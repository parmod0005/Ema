...
class AimlHistoricalOptionCorpusV1Store(private val context: Context) {
    ...
    fun importUris(
        ...
        try {
            uris.forEachIndexed { fileIndex, uri ->
                ...
                input.use { source ->
                    when (name.substringAfterLast('.', "").lowercase(Locale.ROOT)) {
                        "zip" -> recognized = parseZip(
                            name, source, writers, stats, shouldCancel,
                            { msg -> onProgress(ImportProgress(fileIndex, uris.size, stats.rowsRead, stats.accepted, msg)) },
                        ) || recognized
                        "ndjson", "jsonl" -> {
                            ...
                            if (split != null) {
                                ...
                                stats.supported++
                                parseNdjson(
                                    name, split, source, writers.output(split), stats, shouldCancel,
                                    { msg -> onProgress(ImportProgress(fileIndex, uris.size, stats.rowsRead, stats.accepted, msg)) },
                                )
                            }
                        }
            ...
            writers.close(); tempRoot.deleteRecursively(); throw t
        }
        writers.close()
        ...
        if (stats.trainRows <= 0 || stats.validationRows <= 0 || stats.testRows <= 0) {
            ...
            return ImportResult(
                true,
                summary(extraErrors = listOf("Pre-labelled corpus requires train.ndjson, validation.ndjson and test.ndjson with accepted rows")),
                stats.trainRows, stats.validationRows, stats.testRows,
                "Incomplete pre-labelled corpus · train ${stats.trainRows} · validation ${stats.validationRows} · test ${stats.testRows}",
            )
        }
        ...
        tempRoot.deleteRecursively()
        return ImportResult(
            true, summary(), stats.trainRows, stats.validationRows, stats.testRows,
            "Pre-labelled corpus ready · train ${stats.trainRows} · validation ${stats.validationRows} · test ${stats.testRows}",
        )
    }
    ...
    fun summary(extraWarnings: List<String> = emptyList(), extraErrors: List<String> = emptyList()): LocalCorpusSummary {
        ...
        return LocalCorpusSummary(
            filesImported = p.int("files"), supportedFiles = p.int("supported"), rowsRead = p.long("rowsRead"),
            rowsAccepted = p.long("accepted"), rowsRejected = p.long("rejected"), duplicatesRemoved = 0L,
            optionContracts = p.int("contracts"), niftyContracts = p.int("niftyContracts"), sensexContracts = p.int("sensexContracts"),
            ceContracts = p.int("ceContracts"), peContracts = p.int("peContracts"), inferredLotSizeContracts = 0,
        ...
        ZipInputStream(BufferedInputStream(input, BUFFER)).use { zip ->
            while (true) {
                ...
                if (split != null && (base.endsWith(".ndjson", true) || base.endsWith(".jsonl", true))) {
                    ...
                    stats.supported++
                    parseNdjson(
                        "$archiveName/$base", split, NonClosingInputStream(zip), writers.output(split), stats, shouldCancel, onMessage,
                    )
                }
                zip.closeEntry()
            }
    ...
    private fun parseNdjson(
        ...
        while (true) {
            if (shouldCancel()) error("Corpus import cancelled")
            val token = try { reader.peek() } catch (_: Throwable) { break }
            if (token == JsonToken.END_DOCUMENT) break
            if (token != JsonToken.BEGIN_OBJECT) { reader.skipValue(); stats.rejected++; continue }
            stats.rowsRead++
            val record = try { readJsonRecord(reader, stats) } catch (_: Throwable) { null }
            if (record == null) stats.rejected++ else {
                writeRecord(output, record)
                stats.accepted++; localAccepted++
                when (split) {
                ...
                when (proxyEngine(record)) { 2 -> stats.e2Rows++; 3 -> stats.e3Rows++; else -> stats.e1Rows++ }
                ...
                stats.mfe5Sum += record.mfe5; stats.mae5Sum += record.mae5; stats.net5Sum += netReturn5(record)
            }
            ...
            if (stats.rowsRead % PROGRESS_EVERY == 0L) {
                output.flush(); onMessage("$name · ${stats.rowsRead} rows read · ${stats.accepted} accepted")
            }
        }
        output.flush(); onMessage("$name complete · $localAccepted accepted")
    }

    private fun readJsonRecord(reader: JsonReader, stats: MutableStats): Record? {
        var schema = ""; var executionAuthority = false; var capturedAt = ""; var market = ""; var expiry = ""
        var instrumentKey = ""; var optionType = ""; var strike = Double.NaN; var lotSize = 0; var spot = Double.NaN
        var strikeStep = Double.NaN; var signedMoneyness = Double.NaN; var open = Double.NaN; var high = Double.NaN
        var low = Double.NaN; var close = Double.NaN; var volume = Double.NaN; var oi = Double.NaN
        var future1 = Double.NaN; var future3 = Double.NaN; var future5 = Double.NaN; var future15 = Double.NaN
        ...
        while (reader.hasNext()) {
            when (reader.nextName()) {
                "schema" -> schema = nextString(reader); "execution_authority" -> executionAuthority = nextBoolean(reader)
                "captured_at" -> capturedAt = nextString(reader); "market" -> market = nextString(reader); "expiry" -> expiry = nextString(reader)
                "instrument_key" -> instrumentKey = nextString(reader); "option_type" -> optionType = nextString(reader).uppercase(Locale.ROOT)
                "strike" -> strike = nextDouble(reader); "lot_size" -> lotSize = nextDouble(reader).toInt(); "spot" -> spot = nextDouble(reader)
                "strike_step" -> strikeStep = nextDouble(reader); "signed_moneyness_steps" -> signedMoneyness = nextDouble(reader)
                "open" -> open = nextDouble(reader); "high" -> high = nextDouble(reader); "low" -> low = nextDouble(reader); "close" -> close = nextDouble(reader)
                "volume" -> volume = nextDouble(reader); "oi" -> oi = nextDouble(reader)
                "future_return_1m" -> future1 = nextDouble(reader); "future_return_3m" -> future3 = nextDouble(reader)
                "future_return_5m" -> future5 = nextDouble(reader); "future_return_15m" -> future15 = nextDouble(reader)
                "mfe_1m" -> mfe1 = nextDouble(reader); "mfe_3m" -> mfe3 = nextDouble(reader); "mfe_5m" -> mfe5 = nextDouble(reader); "mfe_15m" -> mfe15 = nextDouble(reader)
                "mae_1m" -> mae1 = nextDouble(reader); "mae_3m" -> mae3 = nextDouble(reader); "mae_5m" -> mae5 = nextDouble(reader); "mae_15m" -> mae15 = nextDouble(reader)
        ...
        val required = doubleArrayOf(strike, spot, strikeStep, signedMoneyness, open, high, low, close, volume, oi, future1, future3, future5, future15, mfe1, mfe3, mfe5, mfe15, mae1, mae3, mae5, mae15)
        if (required.any { !it.isFinite() } || lotSize <= 0 || open <= 0.0 || high <= 0.0 || low <= 0.0 || close <= 0.0) return null
        ...
        stats.contracts += "$contractIdentity|$optionType"; stats.expiries += expiry
        return Record(timestampMs, expiryEpochDay, index, optionType, lotSize, open, high, low, close, volume, oi, spot, strike, strikeStep, signedMoneyness, future1, future3, future5, future15, mfe1, mfe3, mfe5, mfe15, mae1, mae3, mae5, mae15)
    }
    ...
    private fun activateAtomically(newData: File, newStats: File) {
        ...
        val backupData = File(backup, "data"); val backupStats = File(backup, "stats.properties")
        val hadData = dataDir.exists() && dataDir.listFiles()?.isNotEmpty() == true; val hadStats = statsFile.isFile
        ...
        if (hadStats && !statsFile.renameTo(backupStats)) {
            if (backupData.exists()) backupData.renameTo(dataDir); error("Could not preserve previous corpus metadata before activation")
        }
        try {
            dataDir.deleteRecursively(); check(newData.renameTo(dataDir)) { "Could not activate compact historical corpus cache" }
            if (statsFile.exists()) statsFile.delete(); check(newStats.renameTo(statsFile)) { "Could not activate historical corpus metadata" }
            backup.deleteRecursively()
        } catch (t: Throwable) {
            dataDir.deleteRecursively(); statsFile.delete()
            if (backupData.exists()) backupData.renameTo(dataDir); if (backupStats.exists()) backupStats.renameTo(statsFile)
            throw t
        }
    }

    private fun newWriter(file: File): DataOutputStream {
        file.parentFile?.mkdirs(); return DataOutputStream(BufferedOutputStream(FileOutputStream(file), BUFFER)).also { it.writeUTF(MAGIC) }
    }

    private fun writeRecord(out: DataOutputStream, r: Record) {
        out.writeLong(r.timestampMs); out.writeInt(r.expiryEpochDay); out.writeByte(r.index.ordinal); out.writeByte(if (r.optionType == "CE") 0 else 1); out.writeInt(r.lotSize)
        doubleArrayOf(r.open, r.high, r.low, r.close, r.volume, r.oi, r.spot, r.strike, r.strikeStep, r.signedMoneynessSteps, r.future1, r.future3, r.future5, r.future15, r.mfe1, r.mfe3, r.mfe5, r.mfe15, r.mae1, r.mae3, r.mae5, r.mae15).forEach { out.writeFloat(it.toFloat()) }
    }

    private fun readRecord(input: DataInputStream): Record {
        val timestamp = input.readLong(); val expiry = input.readInt(); val indexOrdinal = input.readByte().toInt().coerceIn(0, MarketIndex.entries.lastIndex)
        val side = if (input.readByte().toInt() == 0) "CE" else "PE"; val lot = input.readInt(); val v = DoubleArray(22) { input.readFloat().toDouble() }
    ...
    private fun saveStats(file: File, s: MutableStats) {
        ...
        val sensexContracts = s.contracts.size - niftyContracts; val ceContracts = s.contracts.count { it.endsWith("|CE") }; val peContracts = s.contracts.count { it.endsWith("|PE") }
        val p = Properties().apply {
            setProperty("schema", ROW_SCHEMA); setProperty("files", s.files.toString()); setProperty("supported", s.supported.toString())
            setProperty("rowsRead", s.rowsRead.toString()); setProperty("accepted", s.accepted.toString()); setProperty("rejected", s.rejected.toString())
            setProperty("trainRows", s.trainRows.toString()); setProperty("validationRows", s.validationRows.toString()); setProperty("testRows", s.testRows.toString())
            setProperty("ceRows", s.ceRows.toString()); setProperty("peRows", s.peRows.toString()); setProperty("e1Rows", s.e1Rows.toString()); setProperty("e2Rows", s.e2Rows.toString()); setProperty("e3Rows", s.e3Rows.toString())
            setProperty("contracts", s.contracts.size.toString()); setProperty("niftyContracts", niftyContracts.toString()); setProperty("sensexContracts", sensexContracts.toString())
            setProperty("ceContracts", ceContracts.toString()); setProperty("peContracts", peContracts.toString()); setProperty("expiries", s.expiries.size.toString())
            setProperty("market", when { niftyContracts > 0 && sensexContracts == 0 -> "NIFTY"; sensexContracts > 0 && niftyContracts == 0 -> "SENSEX"; else -> "MIXED" })
            setProperty("avgMfe5", (s.mfe5Sum / denominator).toString()); setProperty("avgMae5", (s.mae5Sum / denominator).toString()); setProperty("avgNet5", (s.net5Sum / denominator).toString())
            s.minDate?.let { setProperty("fromDate", it.toString()) }; s.maxDate?.let { setProperty("toDate", it.toString()) }
            setProperty("warnings", s.warnings.distinct().takeLast(12).joinToString("\n"))
        }
        file.parentFile?.mkdirs(); FileOutputStream(file).use { p.store(it, "VARDHANI pre-labelled historical option corpus") }
    }
    ...
    private fun splitFromName(name: String): Split? = when (name.substringAfterLast('/').lowercase(Locale.ROOT)) {
    ...
    private fun nextString(reader: JsonReader): String = when (reader.peek()) {
        JsonToken.STRING, JsonToken.NUMBER -> reader.nextString(); JsonToken.NULL -> { reader.nextNull(); "" }; else -> { reader.skipValue(); "" }
    }
    private fun nextDouble(reader: JsonReader): Double = when (reader.peek()) {
        JsonToken.NUMBER -> reader.nextDouble(); JsonToken.STRING -> reader.nextString().toDoubleOrNull() ?: Double.NaN; JsonToken.NULL -> { reader.nextNull(); Double.NaN }; else -> { reader.skipValue(); Double.NaN }
    }
    private fun nextBoolean(reader: JsonReader): Boolean = when (reader.peek()) {
        JsonToken.BOOLEAN -> reader.nextBoolean(); JsonToken.STRING -> reader.nextString().equals("true", true); JsonToken.NULL -> { reader.nextNull(); false }; else -> { reader.skipValue(); false }
    }
    ...
    companion object {
        ...
        fun proxyEngine(r: Record): Int {
            val range = (r.high - r.low).coerceAtLeast(0.01); val body = abs(r.close - r.open) / range
            return when { abs(r.signedMoneynessSteps) <= 1.5 && body >= 0.55 -> 3; r.volume > 0.0 && r.oi > 0.0 && abs(r.signedMoneynessSteps) <= 2.5 -> 2; else -> 1 }
        }
        fun netReturn5(r: Record): Double {
            val rawReturn = when { r.mae5 <= -STOP_RETURN -> -STOP_RETURN; r.mfe5 >= TARGET_RETURN -> TARGET_RETURN; else -> r.future5 }
            val deployed = (r.close * r.lotSize).coerceAtLeast(1.0); val costs = 2.0 * SLIPPAGE_EACH_SIDE + FLAT_ROUND_TRIP_COST / deployed
