...
class V76ScalperEngine {
    ...
    fun evaluate(oneMinute: List<Bar>, optionChain: List<OptionQuote>, spot: Double, vix: Double): Evaluation {
        if (oneMinute.size < MIN_READY_1M_CANDLES) {
            return wait("1m candles ${oneMinute.size}/$MIN_READY_1M_CANDLES", "V7.6 WARM-UP")
        }
        ...
        if (entryBars.size < 30 || setupBars.size < 35 || biasBars.size < 45) {
            return wait("V7.6 multi-timeframe indicators warming", "V7.6 WARM-UP")
        }
    ...
    private fun evaluateSetup(rows: List<Row>, direction: String): Pair<String?, String> {
        ...
        return when { breakout -> "BREAKOUT" to "3m Bollinger expansion breakout"; pullback -> "PULLBACK" to "3m EMA/VWAP trend pullback"; else -> null to "Waiting for 3m pullback or fresh breakout" }
    }
    ...
    private fun sessionVwap(bars: List<Bar>): List<Double> {
        ...
        var day = Long.MIN_VALUE; var pv = 0.0; var w = 0.0
        bars.forEachIndexed { i, b ->
            ...
            pv += tp * weight; w += weight; out[i] = if (w > 0) pv / w else b.close
        }
        return out
    }
    ...
    private fun emaSeries(v: List<Double>, period: Int): List<Double> {
        ...
        val k = 2.0 / (period + 1.0); val out = MutableList(v.size) { 0.0 }; var e = v.first(); out[0] = e
        for (i in 1 until v.size) { e = v[i] * k + e * (1.0 - k); out[i] = e }
        return out
    }

    private fun rsiSeries(v: List<Double>, period: Int): List<Double> {
        val out = MutableList(v.size) { 50.0 }; if (v.size < 2) return out
    ...
    private fun atrSeries(b: List<Bar>, period: Int): List<Double> {
        val out = MutableList(b.size) { 0.0 }; var a = 0.0
        for (i in b.indices) {
            val tr = if (i == 0) b[i].high - b[i].low else max(b[i].high - b[i].low, max(abs(b[i].high - b[i - 1].close), abs(b[i].low - b[i - 1].close)))
            a = if (i == 0) tr else a + (tr - a) / period; out[i] = a
        }
        return out
    }

    private fun adxSeries(b: List<Bar>, atr: List<Double>, period: Int): Triple<List<Double>, List<Double>, List<Double>> {
        val adx = MutableList(b.size) { 0.0 }; val plus = MutableList(b.size) { 0.0 }; val minus = MutableList(b.size) { 0.0 }
        var p = 0.0; var m = 0.0; var dxSmooth = 0.0
        for (i in 1 until b.size) {
            val up = b[i].high - b[i - 1].high; val down = b[i - 1].low - b[i].low
            val pdm = if (up > down && up > 0) up else 0.0; val mdm = if (down > up && down > 0) down else 0.0
            p += (pdm - p) / period; m += (mdm - m) / period
            if (atr[i] > 0) { plus[i] = 100 * p / atr[i]; minus[i] = 100 * m / atr[i] }
            val den = plus[i] + minus[i]; val dx = if (den > 0) 100 * abs(plus[i] - minus[i]) / den else 0.0
            dxSmooth += (dx - dxSmooth) / period; adx[i] = dxSmooth
        }
        return Triple(adx, plus, minus)
    }

    private fun bollinger(v: List<Double>, i: Int, period: Int, std: Double): Triple<Double, Double, Double> {
        val s = v.subList(max(0, i - period + 1), i + 1); val mean = s.average(); val sd = sqrt(s.map { (it - mean) * (it - mean) }.average())
        val up = mean + std * sd; val dn = mean - std * sd; val width = if (mean != 0.0) (up - dn) / mean else 0.0
        return Triple(up, dn, width)
    }

    private fun rollingMedianWidth(v: List<Double>, i: Int, period: Int): Double {
        val widths = (max(0, i - 19)..i).map { bollinger(v, it, period, 2.0).third }
        return median(widths)
    }

    private fun median(v: List<Double>): Double { if (v.isEmpty()) return 0.0; val s = v.sorted(); return if (s.size % 2 == 1) s[s.size / 2] else (s[s.size / 2 - 1] + s[s.size / 2]) / 2.0 }
    ...
    private fun summarizeOptionChain(chain: List<OptionQuote>, spot: Double): Map<String, Double> {
        ...
        val calls = chain.filter { it.type == "CE" }; val puts = chain.filter { it.type == "PE" }
        val callOi = calls.sumOf { it.openInterest.toDouble() }; val putOi = puts.sumOf { it.openInterest.toDouble() }
        val callChange = calls.sumOf { max(it.changeInOpenInterest, 0L).toDouble() }; val putChange = puts.sumOf { max(it.changeInOpenInterest, 0L).toDouble() }
        ...
        return mapOf("available" to 1.0, "pcr" to if (callOi > 0) putOi / callOi else 0.0, "changePcr" to if (callChange > 0) putChange / callChange else 0.0, "callResistance" to resistance, "putSupport" to support)
    }

    private fun localMinuteOfDay(ts: Long): Int { val z = Instant.ofEpochMilli(ts).atZone(ZoneId.of("Asia/Kolkata")); return z.hour * 60 + z.minute }
    private fun wait(reason: String, setup: String, score: Int = 0) = Evaluation(SignalSnapshot(SignalAction.WAIT, (score * 100 / 11).coerceIn(0, 100), TrendDirection.NEUTRAL, null, null, null, listOf(reason), setup), score = score)

    companion object {
        const val MIN_READY_1M_CANDLES = 220
        const val MIN_OPTION_PREMIUM = 35.0
        const val MAX_OPTION_PREMIUM = 650.0
        const val TARGET_ABS_DELTA = 0.52
        const val PAPER_SLIPPAGE_BPS = 8.0
        const val PAPER_FIXED_COST_PER_TRADE = 45.0
        const val PAPER_EXTRA_PARTIAL_EXIT_COST = 20.0
        const val FAST_STOP_PERCENT = 12.0
        const val FAST_TARGET_PERCENT = 16.0
        const val FAST_TRAIL_TRIGGER_PERCENT = 6.0
        const val FAST_TRAIL_DISTANCE_PERCENT = 5.0
        const val FAST_MAX_HOLD_MINUTES = 12
        const val PULLBACK_STOP_PERCENT = 14.0
        const val PULLBACK_TARGET_PERCENT = 20.0
        const val PULLBACK_TRAIL_TRIGGER_PERCENT = 7.0
        const val PULLBACK_TRAIL_DISTANCE_PERCENT = 5.0
        const val PULLBACK_MAX_HOLD_MINUTES = 20
        const val PROFIT_LOCK_TRIGGER_PERCENT = 4.0
        const val PROFIT_LOCK_TRIGGER_GROSS_INR = 750.0
        const val PROFIT_LOCK_MIN_PERCENT = 1.0
        const val PROFIT_LOCK_RETAIN_RATIO = 0.45
        const val TARGET1_PARTIAL_FRACTION = 0.50
        const val RUNNER_RETAIN_RATIO = 0.80
        const val RUNNER_BREAKOUT_MAX_HOLD_MINUTES = 30
        const val RUNNER_PULLBACK_MAX_HOLD_MINUTES = 40
        const val MAX_TRADES_PER_DAY_PER_INDEX = 3
        const val SAME_DIRECTION_COOLDOWN_MINUTES = 8
        const val REVERSAL_COOLDOWN_MINUTES = 0
        const val REVERSAL_MIN_SCORE = 8
        const val MAX_DAILY_LOSS_INR_PER_INDEX = -3500.0
