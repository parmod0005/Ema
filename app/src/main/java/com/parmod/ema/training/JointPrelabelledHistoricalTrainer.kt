...
/** Joint NIFTY+SENSEX pre-labelled research with one shared chronological window. */
class JointPrelabelledHistoricalTrainer(
    ...
    fun run(
        ...
        val markets = MarketIndex.entries.toSet()
        onProgress(HistoricalCorpusTrainer.Progress("JOINT_WINDOW_PLAN", 0, 1, "Planning ${PrelabelledTrainingWindowPlan.label(months)} BOTH chronological window · shared timestamps"))
        val plan = PrelabelledTrainingWindowPlan.build(store, months, markets, shouldCancel)
        val analysis = PrelabelledTrainingWindowPlan.analyze(store, plan, shouldCancel)

        val missingWindowMarket = MarketIndex.entries.firstOrNull { m ->
            (analysis.trainByMarket[m] ?: 0L) < MIN_MARKET_TRAIN_ROWS ||
                (analysis.calibrationByMarket[m] ?: 0L) < MIN_MARKET_CALIBRATION_ROWS ||
                (analysis.scoringByMarket[m] ?: 0L) < MIN_MARKET_SCORING_ROWS ||
                (analysis.testByMarket[m] ?: 0L) < MIN_MARKET_TEST_ROWS
        }
        if (analysis.trainRows < MIN_TRAIN_ROWS || analysis.calibrationRows < MIN_CALIBRATION_ROWS ||
            analysis.scoringRows < MIN_SCORING_ROWS || analysis.testRows < MIN_TEST_ROWS || missingWindowMarket != null) {
            return result(
                monthsLabel, plan, analysis, 0, null, false, false, null, null, null,
                listOf("${plan.label} BOTH evidence insufficient after chronological embargo${missingWindowMarket?.let { " · ${it.name} lacks one or more roles" } ?: ""}"),
        ...
        onProgress(
            HistoricalCorpusTrainer.Progress(
                "JOINT_WINDOW_READY", 1, 1,
                "${plan.label} ${plan.fromDate}→${plan.toDate} · shared train ${analysis.trainRows} · calibration ${analysis.calibrationRows} · scoring ${analysis.scoringRows} · locked test ${analysis.testRows} · NIFTY/SENSEX both required",
            ),
        )
        ...
        fun batch(hypers: List<NumericalMetaBrain.HyperParameters>, generation: Int, guidance: HistoricalAdaptiveCandidateSearch.Guidance?) {
            ...
            onProgress(HistoricalCorpusTrainer.Progress("${prefix}_CALIBRATION", 0, sampledCalibration.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(), "Timestamp-early BOTH VALIDATION · one shared policy must be viable overall + NIFTY + SENSEX"))
            ...
            brains.forEachIndexed { i, brain ->
                ...
                val policy = BinaryTrainingPolicy.calibrateJoint(overallCalibration[i], marketCalibration[i].values, current)
                calibrated[i] = policy
                brain.configure(current.copy(takeThreshold = policy.takeThreshold, rejectThreshold = policy.rejectThreshold), bumpVersion = false)
            }
            ...
            val productionMarket = MarketIndex.entries.associateWith { Acc() }.toMutableMap()
            ...
            PrelabelledTrainingWindowPlan.forEach(store, plan.scoring, validationStride, shouldCancel) { r ->
                ...
                productionMarket.getValue(s.index).add(pp, s)
                val fold = AimlHistoricalOptionCorpusV1Trainer.timeFold(r.timestampMs, plan.scoring.fromMs, plan.scoring.toMs, FOLDS)
                ...
                scoringUsed++
                if (scoringUsed % PROGRESS_ROWS == 0L) onProgress(HistoricalCorpusTrainer.Progress("${prefix}_SCORING", scoringUsed.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(), sampledScoring.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(), "BOTH G$generation · later scoring $scoringUsed/$sampledScoring · policy frozen · TEST locked"))
            }

            val pm = productionAcc.metrics()
            val pBy = productionMarket.mapValues { it.value.metrics() }
            hypers.indices.forEach { i ->
                ...
                repeat(FOLDS) { f ->
                    ...
                    if (c.labels > 0 && p.labels > 0) { foldsRun++; if (score(c, p) > 0.0) foldsWon++ }
                }
                val policy = calibrated[i]
                val aggregateScore = score(cm, pm) + 0.08 * ((if (foldsRun == 0) 0.0 else foldsWon.toDouble() / foldsRun) - 0.50) + 0.10 * (policy?.score ?: 0.0)
                ...
                val summary = HistoricalCorpusTrainer.CandidateEvaluation(HistoricalAdaptiveCandidateSearch.bounded(hyper), foldsRun, foldsWon, cm, pm, aggregateScore, foldsRun >= 3 && foldsWon >= ceil(foldsRun * 0.75).toInt() && aggregateScore > 0.0)
                val cBy = candidateMarket[i].mapValues { it.value.metrics() }
                val governance = DualMarketHistoricalGovernance.evaluateDevelopment(cm, pm, cBy, pBy, analysis.coverage, analysis.windowRows.coerceAtMost(Int.MAX_VALUE.toLong()).toInt())
                evaluations += JointEvaluation(summary, cBy, pBy, governance)
            }
        }
        ...
        if (best == null || !best.summary.robust || !best.governance.passed) {
            return result(
                monthsLabel, plan, analysis, evaluations.size, best?.summary, false, false, null, null, null, emptyList(),
                "${plan.label} BOTH search exhausted G$generation · ${best?.governance?.label ?: "CLOSED"}: ${best?.governance?.reasons?.joinToString("; ") ?: "no candidate"} · locked TEST never opened.",
            )
        }
        ...
        var testUsed = 0L
        onProgress(HistoricalCorpusTrainer.Progress("JOINT_PRELABELLED_LOCKED_TEST", 0, analysis.testRows.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(), "Opening shared ${plan.label} chronological TEST ONCE · NIFTY + SENSEX required"))
        PrelabelledTrainingWindowPlan.forEach(store, plan.test, 1, shouldCancel) { r ->
            val s = sample(r)
            val cp = champion.predict(s.features); val pp = production.predict(s.features)
            cAcc.add(cp, s); pAcc.add(pp, s)
            cMarket.getValue(s.index).add(cp, s); pMarket.getValue(s.index).add(pp, s)
            testUsed++
            if (testUsed % 100_000L == 0L) onProgress(HistoricalCorpusTrainer.Progress("JOINT_PRELABELLED_LOCKED_TEST", testUsed.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(), analysis.testRows.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(), "BOTH locked TEST · $testUsed/${analysis.testRows}"))
        }
        val cm = cAcc.metrics(); val pm = pAcc.metrics()
        val cBy = cMarket.mapValues { it.value.metrics() }; val pBy = pMarket.mapValues { it.value.metrics() }
        ...
        val note = "${plan.label} ${plan.fromDate}→${plan.toDate} · BOTH NIFTY+SENSEX share derived chronological 70/15/15 roles across all storage shards · ${PrelabelledTrainingWindowPlan.EMBARGO_MINUTES}m label embargo · one shared cost-aware policy · locked TEST opened once · deferred causal materialization · governance ${governance.label}: ${governance.reasons.joinToString("; ")} · NIFTY test ${cBy[MarketIndex.NIFTY]?.labels ?: 0} · SENSEX test ${cBy[MarketIndex.SENSEX]?.labels ?: 0}."
        return result(monthsLabel, plan, analysis, evaluations.size, best.summary, true, governance.passed, cm, pm, state, emptyList(), note)
    }
    ...
    private fun selectBest(evals: List<JointEvaluation>): JointEvaluation? {
        ...
        return (passed.ifEmpty { evals }).maxByOrNull { e ->
            ...
            for (m in MarketIndex.entries) {
                val c = e.candidateByMarket[m] ?: HistoricalCorpusTrainer.Metrics(); val p = e.productionByMarket[m] ?: HistoricalCorpusTrainer.Metrics()
    ...
    private fun result(
        ...
        index = MarketIndex.NIFTY, months = months, fromDate = plan.fromDate, toDate = plan.toDate,
        expiries = a.expiries, contractsDownloaded = a.contracts, corpusSamples = a.windowRows.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(), coverage = a.coverage,
    ...
    companion object {
        ...
        private const val MIN_TRAIN_ROWS = 240L
        private const val MIN_CALIBRATION_ROWS = 80L
        private const val MIN_SCORING_ROWS = 120L
        private const val MIN_TEST_ROWS = 100L
        private const val MIN_MARKET_TRAIN_ROWS = 80L
        private const val MIN_MARKET_CALIBRATION_ROWS = 30L
        private const val MIN_MARKET_SCORING_ROWS = 40L
        private const val MIN_MARKET_TEST_ROWS = 40L
        private const val MIN_MARKET_SAMPLED_CALIBRATION = 10L
        private const val MIN_MARKET_SAMPLED_SCORING = 15L
