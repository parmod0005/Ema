...
class SignalEngineV2 {
    ...
    data class Config(
        ...
        val minimumAdx: Double = 20.0,
        val minimumAtrExpansion: Double = 1.05,
    ...
    data class Evaluation(
        ...
        val actionable: Boolean get() = direction != Direction.NEUTRAL && score >= 80
    }

    fun evaluate(bars: List<Bar>, config: Config = Config()): Evaluation {
        val minimumBars = max(config.slowEma + 5, config.adxPeriod * 2 + 2)
        ...
        if (bullish || bearish) { score += 30; reasons += "EMA20/50 trend aligned" }
        if (higherAligned) { score += 20; reasons += "5-minute trend confirmed" }
        if (adx >= config.minimumAdx) { score += if (adx >= 25.0) 20 else 15; reasons += "Trend strength ADX ${adx.toInt()}" }
        if (atrExpansion >= config.minimumAtrExpansion) { score += 15; reasons += "ATR expansion ${format(atrExpansion)}x" }
        if (volumeRatio >= config.minimumVolumeRatio) { score += 10; reasons += "Volume ${format(volumeRatio)}x" }
        if (separationAtr >= 0.30) { score += 10; reasons += "EMA separation confirmed" }
        score = score.coerceIn(0, 100)

        val qualityPass = higherAligned && adx >= config.minimumAdx &&
            atrExpansion >= config.minimumAtrExpansion && separationAtr >= 0.20
        val direction = when {
        ...
        if (direction == Direction.NEUTRAL) reasons += "Quality filters not fully confirmed"
